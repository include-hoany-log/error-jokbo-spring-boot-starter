...
error-jokbo:
  ...
  error-response-class: "com.jichul.jichul.common.exception.JichulErrorResponse"

  # 6. 소스코드 추적 시 비즈니스 예외로 인지하고 파싱할 커스텀 예외 클래스 목록 (필수, 리스트 형태)
  # 이 예외 객체가 new 키워드로 throw 되는 시점을 추적합니다.
  exception-classes:
    - "com.jichul.jichul.common.exception.JichulException"
```
...
```java
package com.jichul.jichul.common.exception;

public enum JichulError {
    ...
    JichulError(String statusCode, String message) {
        this.statusCode = statusCode;
        this.message = message;
    }

    // 💡 라이브러리가 리플렉션으로 호출하여 에러 메시지를 가져옵니다.
    public String getMessage() { return message; }

    // 💡 라이브러리가 리플렉션으로 호출하여 HTTP Status 분기 그룹을 생성합니다. (400, 404, 500 등)
...
Enum 구조를 품고 다닐 우리 프로젝트만의 비즈니스 예외 클래스를 정의합니다. 이 예외 클래스 경로를 `application.yml`의 `exception-classes`에 적어주면 됩니다.

```java
package com.jichul.jichul.common.exception;

public class JichulException extends RuntimeException {
    private final JichulError jichulError;

    public JichulException(JichulError jichulError) {
        super(jichulError.getMessage());
        this.jichulError = jichulError;
    }

    public String getCode() { return jichulError.getErrorCode(); }
}
```
...
@Getter
public class JichulErrorResponse {
...
```java
package com.jichul.jichul.web;

import com.jichul.jichul.common.exception.JichulError;
import com.jichul.jichul.common.exception.JichulException;
...
public class UserRestController {
    ...
    public String getUserInfo() {
        ...
        if (!isUserExist) {
            ...
            // 현재 컨트롤러 메서드(/api/v1/user)와 'NOT_FOUND_USER' 에러 코드를 매핑합니다!
            throw new JichulException(JichulError.NOT_FOUND_USER);
        }
